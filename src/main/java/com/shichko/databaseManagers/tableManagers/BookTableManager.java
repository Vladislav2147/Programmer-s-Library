package com.shichko.databaseManagers.tableManagers;

import com.shichko.book.Book;
import com.shichko.book.Publisher;
...
public class BookTableManager implements ITableManager<Book> {
    ...
    public Book getInstanceById(int id, Connection connection) throws SQLException {
        ...
        Optional.ofNullable(resultSet.getObject("publishing_date")).ifPresent((date) -> book.setPublishingDate(LocalDate.parse(date.toString())));

        Optional.ofNullable(resultSet.getObject("copies")).ifPresent((copies) -> book.setCopiesAmount((int)copies));

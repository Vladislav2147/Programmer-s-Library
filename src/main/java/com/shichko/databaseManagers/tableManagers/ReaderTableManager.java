...
public class ReaderTableManager implements ITableManager<Reader> {
    ...
    public Reader getInstanceById(int id, Connection connection) throws SQLException {
        ...
        Optional.ofNullable(resultSet.getObject("birth_year")).ifPresent((year) -> reader.setBirthYear(Year.of((int)year)));

        Optional.ofNullable(resultSet.getObject("phone")).ifPresent((phone) -> reader.setPhone(phone.toString()));
